...
void signalhandler(int sig) {
    if (sig == SIGALRM) {
        struct tcb* temp = list_first_entry(&tcbs, struct tcb, list);
        //sigprocmask(SIG_BLOCK, &(current_tcb->context->uc_sigmask), NULL);
...
void next_tcb() {
    ...
    if (thread_policy == FIFO) {
        ...
        if (current_tcb->lifetime <= 0) current_tcb->state = TERMINATED; else current_tcb->state = READY;
    ...
    if (current_tcb == next && current_tcb->tid == -1) {
        sigprocmask(SIG_BLOCK, &mask, NULL);
    }

    else //(current_tcb != next){
    {
        ...
        //
        if (thread_policy == RR || thread_policy == PRIO) {
...
struct tcb* rr_scheduling(struct tcb* next) {
   // fprintf(stderr , "\n===== RR====\n");
    ...
    if (n_tcbs <= 1) {
        ...
        for (int i = 0; i < n_tcbs-1; i++) {
            if (temp->tid == last_entry->tid) temp = list_first_entry(&tcbs, struct tcb, list);
            else temp = list_next_entry(temp, list);
            //fprintf(stderr, "tid : %d , lifetime = %d , state : %d\n", temp->tid, temp->lifetime, temp->state);
            if ((temp != NULL) && (temp->state == READY) && (temp->lifetime > 0)) return temp;
        }
        temp = list_first_entry(&tcbs, struct tcb, list);
        return  temp;
     }
 ...
 **************************************************************************************/
struct tcb* prio_scheduling(struct tcb* next) {
    struct tcb* prio = list_first_entry(&tcbs, struct tcb, list);
    struct tcb* temp = NULL;
    if (n_tcbs <= 1) {
        temp = list_first_entry(&tcbs, struct tcb, list);
        return temp;
    ...
    list_for_each_entry(temp, &tcbs, list) {
        //printf("%d state  : %d\n", temp->tid, temp->state);
        if ((temp != NULL) && (temp->state == READY)) {

            if (temp->priority > prio->priority) prio = temp;
            //printf("%d state  : %d\n", temp->tid, temp->state);
        }
    }
    return prio;
...
int uthread_create(void* stub(void*), void* args) {
    ...
    if (pre)sigemptyset(&(new_tcb->context->uc_sigmask));
    else sigaddset(&(new_tcb->context->uc_sigmask), SIGALRM);
    //new_tcb->context->uc_sigmask = mask;
