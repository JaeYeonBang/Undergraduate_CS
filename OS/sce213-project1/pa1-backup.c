...
void dump_stack(void)
{
	...
	//list_for_each_entry(ptr, &history, list) {
	//	fprintf(stderr, "%2d: %s", ptr->index, ptr->string);

	//}//	Print out values in this form */
	for (ptr = list_first_entry(&history, __typeof__(*ptr), list);	\
		& ptr->list != (&history);					\
		ptr = list_next_entry(ptr, list))
		{	
		if (ptr->string[strlen(ptr->string) - 1] == '\n')
		fprintf(stderr, "%2d: %s", ptr->index, ptr->string);
		else fprintf(stderr, "%2d: %s\n", ptr->index, ptr->string);
		}
...
/*		struct entry * ptr;
	...
	//		exit(3);
		...
		struct entry * ptr;
		int from_token = atoi(tokens[1]);

		list_for_each_entry(ptr,&history,list) {
			
			if(from_token == ptr->index){
				if((pid = fork()) == 0) {
				
			__process_command(ptr->string);
			exit(3);
		} else {
