...
#define HDRPOINT_SIZE HDRSIZE/32
...
void *my_malloc(size_t size)
{
    ...
    header_t* last_header = list_last_entry(&free_list, header_t, list);
    if (g_algo == FIRST_FIT) {
    ...
    if (!found_empty) {     /// not found empty
        ...
        if (!last_header->free) {
            ...
            list_add(&(new_header)->list, &(list_prev_entry(last_header,list))->list);
        }
        ...
        if (empty_header->size > ALIGNMENT * blocks) {
            ...
            void* new_header_ptr = (void*)empty_header + HDRSIZE +ALIGNMENT * blocks;
            ((header_t*)(new_header_ptr))->free = true;
            ((header_t*)(new_header_ptr))->size = empty_header->size - ALIGNMENT * blocks - HDRSIZE;
            list_add(&((header_t*)(new_header_ptr))->list, &empty_header->list);
            empty_header->size = ALIGNMENT * blocks;
        }

        return empty_header + HDRPOINT_SIZE;
    }
...
void *my_realloc(void *ptr, size_t size)
{
    ...
    void* new_ptr = NULL;
    //header_t* next_header = list_next_entry(header, list);
